...
public class StoreController : Controller
{
  ...
  // ========================================================================
  // POST: Store/CancelOrder - CANCELAR PEDIDO (Solo PENDING)
  // ========================================================================
  // Permite al cliente cancelar su pedido si aún está en estado PENDING.
  // Si el pedido ya está en ON_PREPARATION o DELIVERED, no se puede cancelar.
  // ========================================================================
  [HttpPost]
  [Authorize]
  [ValidateAntiForgeryToken]
  public async Task<IActionResult> CancelOrder(int id)
  {
    ...
    // 🔒 4. SEGURIDAD: Verificar que el pedido pertenece al usuario
    if (order.CustomerId != customer.CustomerId)
    {
      TempData["Error"] = "No tienes permiso para cancelar este pedido.";
      return RedirectToAction(nameof(MyOrders));
    }

    // ❌ 5. Verificar que el pedido está en PENDING
    if (order.Status != "pending")
    {
      TempData["Error"] =
        $"No puedes cancelar un pedido que ya está '{order.Status.ToUpper()}'. Solo pedidos PENDIENTES pueden ser cancelados.";
      return RedirectToAction(nameof(MyOrders));
    }

    // ✅ 6. Cambiar el estado a CANCELLED
    ...
    // 🎉 7. Mensaje de éxito
    TempData["Success"] = $"Pedido #{order.OrderId} cancelado exitosamente. No se descontará inventario.";
